        ...
        if (fsmServer==0)
        {
        	...
        	switch (buffer[0])
        	{
				...
				// Si le nombre de joueurs atteint 4, alors on peut lancer le jeu
                                ...
                                if (nbClients==4)
								{
										for(int i = 0; i < 4; i++){	//code rajouté
												sprintf(reply, "%d %d %d %d", deck[3*i], deck[(3*i)+1], deck[(3*i)+2], i);
												sendMessageToClient(tcpClients[i].ipAddress, tcpClients[i].port, reply);	// On envoie ses cartes au joueur 0-4, ainsi que la ligne qui lui correspond dans tableCartes
										}
		...
		else if (fsmServer==1)
		{
			switch (buffer[0])
			{
				case 'G':	// Guilty => num du joueur + accuse qui? [0-12]
					...
					break;
				case 'O':	// Object => num du joueur objet [0-7]
					// RAJOUTER DU CODE ICI
					break;
				case 'S':	// Symbol => num du joueur + 
					// RAJOUTER DU CODE ICI
					break;
				default:
					break;
